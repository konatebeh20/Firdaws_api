...
#     # ========== GET, PUT, DELETE (Restent identiques à ton code) ==========
#     # ... (Garde ton code tel quel pour ces méthodes)
...
#                 # --- Traitement de la sauvegarde physique ---
#                 # Exemple de chemin local (à adapter selon ta configuration)
#                 # upload_path = os.path.join('uploads', 'documents', filename)
#                 # file.save(upload_path)
                
#                 # Données de simulation (remplace par tes calculs réels)
#                 file_url = f"/uploads/documents/{filename}"
#                 file_size_bytes = 1048576  # Reçu via le traitement du fichier ou os.path.getsize
                
#                 # Préparation via le helper mis à jour
#                 doc_data = DocumentHelper.prepare_data(
#                     data=data,
#                     filename=filename,
#                     size_bytes=file_size_bytes,
...
#                 # Si pas de fichier physique fourni, on extrait les valeurs par défaut du JSON
#                 doc_data = {
#                     'title': data.get('title', '').strip(),
#                     'description': data.get('description', '').strip(),
#                     'author': data.get('author', 'Anonyme').strip(),
